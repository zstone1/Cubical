...
3. A bit of general machinery in WedgeHom.lean, any such presheaf P, with a unique 0-cell induces an equivalence 
   ...
   That is, maps into P split over the wedges uniformly. This is yoneda shenanigans, plus the fact that the gluings are forced at the junction points.
...
Even better, the construction for the braid trick is reproducible from the Ch* side _without_ going through the Sal machinery. Essentially, every morphism of Ch*(K) is a permutation identifying where two runs disagree. These cannot dissagree in the same place twice, so we get compositionality in the _Garside Germ Presentation_ of the braid group. Which gives us a functor
