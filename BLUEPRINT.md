...
- Functoriality of the **degree-zero** polygraph in `K`. `Ch(K)[W⁻¹]` is functorial (`chLocMap`),
  and so are the localized cut polygraph (`chCutLocFunctor`) and the contracted one
  (`chRunFunctor`); the restriction to degree zero is built pointwise, and `Polygraph.sub` carries
  no functoriality.
