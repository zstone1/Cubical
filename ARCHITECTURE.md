...
| 9 | …and presented by the **base's own cells**, at whichever presentation the base carries: a 1-cell is a generator of `p` acting on a run, a 2-cell a relation of `p` holding there | `presentsChainsSliceColimit K p hp`, instantiated as `presentsChainsGarsideColimit K` and `presentsChainsArtinColimit K` | `Concurrency/Presentation/SliceInherit.lean` |
...
not a normal form — the target is not thin, so `presentsGlue` builds an inverse `Ψ` with
...
| **A thin category is presented by any spanning quiver** | `Presents.ofThin` — relating *every* parallel pair of words leaves no word problem, so a presentation of a preorder is a spanning family of generators on a covering family of 0-cells and nothing else (`Polygraph.thin`; soundness, completeness and faithfulness are all free).  At the cube that is `cubePresentation n : Presents (Polygraph.thin (CubeStep n)) ((W (□n)).Localization)` — 0-cells the runs, generators the **atom steps**, one per descent — since `Ch (□n)[W⁻¹]` is a poset (`locCube_isThin`, and `locCubeWeakOrder` names it as the weak Bruhat order): `exists_word` read as a path is fullness, and the merge out of a chain's own run (`classRunIso`) is essential surjectivity | `Machinery/Presentation/Basic.lean`, `Concurrency/Presentation/CubePresentation.lean`, `Concurrency/Merge/CubeThin.lean` |
| **The localized slice is presented bead by bead** | `slicePresentation d : Presents (slicePoly d) ((W (⋁d)).Localization)` — `Ch (⋁d)[W⁻¹]` splits as a product over the beads (`locChConsEquiv`), so the polygraph is one copy of the cube's atom steps per bead and the 2-cells are the factors' plus `ProdRel.interchange`: atoms in different beads commute.  The induction step is one line, `Presents.prod` being hypothesis-free.  `locSlice_isThin` says the slice is a poset, so `Presents.ofThin` would also apply — but it would quotient by *every* parallel pair and lose the interchange relations that name why the beads are independent.  `overSlicePresentation` is the same polygraph on `(Ch Zbp/d)[W⁻¹]`.  The induction is `beadPresentation`, run once on an *arbitrary* family of cube presentations: at `cubePresentation` it is the atom steps, at `cubeLocPresentation · p` it is `sliceLocPresentation`, parametric in a presentation `p` of the base — and `chOverBeadPresentation` is where the glue family consumes either | `Concurrency/Presentation/SlicePresentation.lean`, `Machinery/Presentation/Product.lean`, `Concurrency/Merge/WedgeLocalize.lean` |
| **Gluing the slices wants a functor of them, and the slice is its own polygraph** | `runPolyFunctor : Ch Zbp ⥤ Polygraph` — 0-cells `RunOver d` (the runs over `d`), `P.map f := Over.map f`.  Both functor laws are `rfl`: composition in `Ch Zbp` is strictly associative *and* unital, and a 1-cell carries **no data** — `RunStep a b` is the *property* that `a` and `b` agree after `a` crosses one pair and `b` merges (`∃ e t m z, permLen (crossPerm rfl t) = 1 ∧ W Zbp m ∧ t ≫ z = a.hom ∧ m ≫ z = b.hom`), and `P.map` on it is that same square read through `z ≫ f`.  So no crossing permutation is touched and nothing is transported: a *position*-indexed generator would live in `Fin (dimSum d.dims - 1)`, and `dimSum` is preserved by arrows of `Ch Zbp` only **propositionally**, which is what would drag `finCongr` through both laws.  `runLabels.map_ob` is then `rfl`, and `runPoly_hP` — the *equality* of functors `GlueRel.overlap` needs, and what walled the product indexing — is `Functor.ext` with `Subsingleton.elim` on morphisms, since `locOver_isThin` says the localized slice is a poset and `overMapLoc_obj` says `overMapLoc` is `Over.map` on objects.  `presentsChainsRunGlueOf` assembles it: what remains of the glue route is exactly a family of slice presentations naming its own 0-cells | `Concurrency/Presentation/SliceFunctor.lean`, `Machinery/Presentation/Glue.lean` |
| **…and each localized slice is presented by its run-arrows, for every `d`** | `runPresentation : Presents (runPoly d) (((W Zbp).over d).Localization)`, hence `runSlicePresentation` as a family and `presentsChainsRunGlue K`, **unconditional in `K`**.  Two halves.  A *grading*: `weakOver_le_of_loc_hom` — `Machinery/Grading`'s `degLoc` chain, at `weakOver` at an arbitrary base, with `crossOver` for `cross` and nothing in it about the cube.  And one *geometric* fact, `index_crossPerm`: **an arrow permutes each block of its target and no more** — read the target in its own standard chart, where the source's firing order inverts `crossPerm` (`crossPerm_flatten`) and a coarsening's beads are the target's blocks read in that order (`beadOf_of_hom`).  So a descent at `k` puts `k` and `k+1` in one block (`index_adj_eq_of_descent`), which *is* the arrow `zObj (atomComp N k) ⟶ d` (`nonempty_hom_of_index`); and `exists_crossPerm_of_blocks` then realises `σ * adjT k`, because the only pair `adjT k` reorders is that one (`exists_run_mul_adjT`).  Fullness is `exists_cover_of_lt` picking the descent and this realising it, on an induction over `permLen`; essential surjectivity is `runMerge`, which the localization inverts | `Concurrency/Presentation/SliceExchange.lean`, `Concurrency/Grading/ChartHom.lean`, `Concurrency/Merge/CubeFaces.lean` |
| **…and by the base's own cells, at whichever presentation the base carries** | The runs over `d` are a **downward-closed** set of permutations for the right weak order — that is the exchange `exists_runOver_mul_adjT`, iterated (`runSet_of_le`) — so `PosBraid N` acts on them partially by length-additive right multiplication (`weakActionOn`, built with `PosBraid.lift`, never through Artin), `sliceFibre d` is the presheaf on the localized base that the action gives, and its defined part **is the localized slice, reversed** (`definedSliceLoc`): a braid raises the weak order where a slice arrow lowers it, which is the only `ᵒᵖ` in the family.  So `slicePolyFunctor p := (p.elements (sliceFibre ·)).restrictPoly (defined ·) ⋙ opFunctor` lifts the *base's* cells — a 1-cell is a generator of `p` acting on a run, a 2-cell a relation of `p` holding there — and moves when `p` does: `(0, 6, 72)` Artin relations against `(8, 144)` germ ones, where the thin family gives `(4, 54, 9888)` at every `p`.  Functoriality is **lax and not natural**: pushing a run along `f : d' ⟶ d` commutes with the action only where both ends are defined (over `𝟙²` the atom is undefined; over `[2]` it is), which is exactly what a morphism of polygraphs asks and what `PartialFam`/`partialElementsMap` consume.  `hP` is free for the old reason — the 0-cells name their own slice objects (`sliceCellOver`) and pushing them is `Over.map` — and `SliceSkeleton` needs one condition on the base, `StrandSeparated`: one 0-cell per strand count, which every monoid presentation of the braid monoids satisfies | `Concurrency/Presentation/SliceInherit.lean`, `Concurrency/Presentation/SliceFibre.lean`, `Machinery/Braid/WeakAction.lean`, `Machinery/Presentation/Partial.lean` |
| **The word problem is a retraction, slice by slice** | `presentsGlue` *proves* Φ faithful rather than assuming it: `Ψ` is an honest cocone on the slices (`glueRetract`) and `Φ ⋙ Ψ = 𝟭` on the nose (`glueUnit`) — six rewrites, now that the glued polygraph **is** `colimit` and its universal property on presented categories (`glueLift`, `glueInclFun_lift`, `glue_functor_ext`) comes straight from `catHomEquiv`.  The one input is `SliceSkeleton`, a **single field**: exactly one 0-cell names an object isomorphic to each `y : Over d` (`entry`).  `ret`, `iso`, `fix`, `at_injective` and `ret_push` all derive from it, and **naturality comes from the uniqueness**, needing only `hP` — which is why `SliceLabels`/`hL` left the comparison chain entirely.  At `Ch K` the skeleton is `RunOver.eq_of_locIso`: two runs isomorphic in the localized slice are *equal*, the weak order being a grading the localization keeps and an iso pinning it from both sides.  What this does **not** give is a levelwise isomorphism — a localized slice is thin but never skeletal, since a `W`-arrow makes distinct objects isomorphic, and `not_surjective_runSlicePresentation_obj` witnesses the failure (`Localization.Construction` keeps every object of `Over d`, and `Over d` has objects that are not runs).  So the counit is genuinely only an isomorphism, assembled slice by slice through `OverPseudoCocone.descIso` | `Machinery/Presentation/Glue.lean`, `Concurrency/Presentation/SliceExchange.lean` |
| **The lifting datum is an Artin family, not a per-presentation obligation** | `isArtinFamily_cubeAtom n` — the cube's partial atom action satisfies commutation and braid, so it *is* a monoid hom `chartActionAt : PosBraid N →* _` (`PosBraid.liftArtin`).  Landed in the `none`-preserving submonoid and read contravariantly (`IsArtinFamily.op`: the relations are reversal-invariant, so the base's `ᵒᵖ` costs nothing), it becomes a presheaf `cubeFibre` on the localized base — built with **no presentation in sight**.  `Presents.partialElements` then takes any `p`, so `cubeChartGarside` and `cubeChartArtin` are one lemma at two `p`s.  The braid case needs **no 3-cell**: `atomAct` is defined exactly at an *ascent* and multiplies `cross` by `adjT k`, so each three-letter route is defined exactly when its window increases — the same three inequalities in a different order.  Nothing here uses `hasDiamonds_cube` or `CubeFaces`' meet layer, which stay load-bearing for thinness alone | `Concurrency/Presentation/CubeChartAction.lean`, `Machinery/Braid/Artin.lean`, `Machinery/Presentation/Partial.lean` |
| **…and what it presents is the localized cube** | `definedCubeFibreLoc n : (Presents.defined (cubeFibre n) (cubeBot n)).FullSubcategory ≌ (W (□n)).Localization`, so `cubeLocPresentation n p` presents `Ch (□n)[W⁻¹]` for an **arbitrary** presentation `p` of the base.  Two things put a `ᵒᵖ` in the way — the base is presented on `((W Zbp).op).Localization`, and the atoms go *up* the weak order where a localization morphism goes down — and both are removed at once by the order's self-duality `σ ↦ w₀σ` (`WeakOrder.revEquivalence`, on mathlib's `OrderIso.equivalence` and `orderDualEquivalence`); `Fin.revPerm` *is* `w₀`, and `permLen_mul_of_eq_rev` already said so.  A chart over the run *is* its crossing permutation (`crossOnesEquiv`), and a braid is defined at it exactly where it adds all of its own crossings — one length equation, `permLen (crossOnes x) + toAdd (posLen n β) = permLen (crossOnes y)` (`chartActionAt_eq_some_iff`), which by subadditivity of `permLen` forces both that `β` is a simple (`eq_posPerm_of_posLen`) and that the product is length-additive, i.e. exactly the right weak Bruhat order (`chartWeakEquiv`).  The `ᵐᵒᵖ`'s inverse is `matsuLift_op : matsuLift (op ∘ g) σ = op (matsuLift g σ⁻¹)`.  `strandDecomposition.functor` is a `Functor.inv` and is never evaluated on objects: `preDefinedEquiv` transports the *defined* part along `runBase n`, asking covering only of the objects carrying a defined element — which `isEquivalence_pre` cannot ask, since `bot` sits over every object | `Concurrency/Presentation/CubeChartWeakOrder.lean`, `Machinery/Presentation/Partial.lean`, `Machinery/Braid/Matsumoto.lean` |
...
- `SlicePresentation.md` — presenting `Ch(K)[W⁻¹]` by gluing presentations of localized slices:
  the repo dictionary for that programme, the numbered phases, and the landmines each one hit.
  Revised as the work proceeds; nothing else restates it.
