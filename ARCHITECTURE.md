...
| **…and the *defined* part of `∫F`, when lifting is only partial** | A functor with *at most one* lift of each arrow is classified by a presheaf of **partial** functions; since `Par ≃ Set⋆` and `[Cᵒᵖ, Set⋆] = 1/PSh C`, that is an ordinary `G` with a global section `bot`, and the category is `∫G` minus it.  So nothing new is presented: `partialElements = (p.elements G).restrict (defined G bot)`, free because `bot` is **absorbing** — a word reaching it stays there, so a word between defined objects never passes through it (`convex_of_absorbing`).  `bot` unreachable is the total case, where the defined part is `∫` of an honest presheaf (`definedEquiv`) and the restriction discards nothing | `Machinery/Presentation/Partial.lean` |
...
| **The polygraph tensor, and its strictly associative model** | `Polygraph.prod` is not a categorical product: it carries `ProdRel.interchange` precisely so that `presented` takes it to `×`, so it is a **tensor**, and `Presents.prod` is the strong monoidality of `presented : (Polygraph, ⊗) ⥤ (Cat, ×)`.  `Polygraph.pi P` is that tensor over a finite index and `Presents.pi p : Presents (Polygraph.pi P) (∀ i, C i)` its monoidality, hypothesis-free.  It exists because `(P ⊗ Q) ⊗ R` and `P ⊗ (Q ⊗ R)` have 0-cells `(V_P × V_Q) × V_R` and `V_P × (V_Q × V_R)`, so a monoidal functor into `⊗` carries coherence isos exactly where a consumer needs a strict `map_comp` — a tuple over the index removes them, as `List` models the free monoid where nested pairs do not.  Every cell pins its endpoint 0-cells by a `Shift` (a proposition) rather than by a `Function.update` in its index, which does not reduce at a variable index; that is what keeps the whole file transport-free.  Reindexing is `piFunctor : (ι → Polygraph) ⥤ Polygraph`, with `piMap_id` and `piMap_comp` honest equalities.  Completeness is `exists_piNormalForm`: every word sorts by index, running each factor's word once in the order of any duplicate-free list of the indices | `Machinery/Presentation/Pi.lean` |
...
- `Partial.lean` — the partial case of the above: `∫G` minus an absorbing global section
  (`Presents.partialElements`, whose convexity is `convex_of_absorbing`), and `definedEquiv` for
  `bot` unreachable, where the defined part is `∫` of an honest subfunctor.
...
- `Pi.lean` — the same tensor over a finite index, in the strictly associative model:
