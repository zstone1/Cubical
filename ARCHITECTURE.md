...
| **The localized slice is presented bead by bead** | `slicePresentation d : Presents (slicePoly d) ((W (⋁d)).Localization)` — `Ch (⋁d)[W⁻¹]` splits as a product over the beads (`locChConsEquiv`), so the polygraph is one copy of the cube's atom steps per bead and the 2-cells are the factors' plus `ProdRel.interchange`: atoms in different beads commute.  The induction step is one line, `Presents.prod` being hypothesis-free.  `locSlice_isThin` says the slice is a poset, so `Presents.ofThin` would also apply — but it would quotient by *every* parallel pair and lose the interchange relations that name why the beads are independent.  `overSlicePresentation` is the same polygraph on `(Ch Zbp/d)[W⁻¹]` | `Concurrency/Presentation/SlicePresentation.lean`, `Machinery/Presentation/Product.lean`, `Concurrency/Merge/WedgeLocalize.lean` |
...
| **…and what it presents is the localized cube** | `definedCubeFibreLoc n : (Presents.defined (cubeFibre n) (cubeBot n)).FullSubcategory ≌ ((W (□n)).Localization)ᵒᵖ`, so `cubeChartPresentation n p` presents `Ch (□n)[W⁻¹]ᵒᵖ` for an **arbitrary** presentation `p` of the base.  A chart over the run *is* its crossing permutation (`crossOnesEquiv`), and a braid is defined at it exactly where it adds all of its own crossings — one length equation, `permLen (crossOnes x) + toAdd (posLen n β) = permLen (crossOnes y)` (`chartActionAt_eq_some_iff`), which by subadditivity of `permLen` forces both that `β` is a simple (`eq_posPerm_of_posLen`) and that the product is length-additive, i.e. exactly the right weak Bruhat order (`chartWeakEquiv`).  The `ᵐᵒᵖ`'s inverse is `matsuLift_op : matsuLift (op ∘ g) σ = op (matsuLift g σ⁻¹)`.  `strandDecomposition.functor` is a `Functor.inv` and is never evaluated on objects: `preDefinedEquiv` transports the *defined* part along `runBase n`, asking covering only of the objects carrying a defined element — which `isEquivalence_pre` cannot ask, since `bot` sits over every object | `Concurrency/Presentation/CubeChartWeakOrder.lean`, `Machinery/Presentation/Partial.lean`, `Machinery/Braid/Matsumoto.lean` |
