...
| **Schedule space of `Ch(K)`** | HDA event naming; the Galois connection `gc_coneUnion_chainsIn` + good cover `maximalChains_goodCover` | `Schedule/ScheduleSpace.lean` |
...
### `Schedule/` — HDA event naming + the schedule space of `Ch(K)`
`EventNaming`/`EventLocalSystem`/`HDA` (the run-injective HDA labelling), `ChainCone` (chain ↦ open
convex cone), `ScheduleSpace` (the Galois connection + good cover). See `Schedule/README.md`.
...
- **the schedule space / Galois connection / good cover** → `Schedule/ScheduleSpace.lean`
