...
product over a varying index. That is why `slicePoly_hP` is ten lines: its object half is
`slicePresentation_at` — "a 0-cell names its own slice object", one rewrite through the coproduct's
universal property — and its morphism half is `Subsingleton.elim`.
...
| **…and by the base's own cells, at whichever presentation the base carries** | The runs over `d` are a **downward-closed** set of permutations for the right weak order — that is the exchange `exists_runOver_mul_adjT`, iterated (`runSet_of_le`, `weakDown_runSet`) — so they *are* a `GermChart` (`runGermChart d N`), and `sliceLocEquiv d` reads the localized slice over `d` as that chart's order, backwards: a germ step raises the weak order where a slice arrow lowers it, which is the only `ᵒᵖ` in the family.  `p.slicePoly d` is `p`'s germ on the runs, one chart per strand count (`Polygraph.coprod`, since every count but `d`'s own is empty), and `p.dehornoy` presents each — **no `Option`, no absorbing point, no choice**, a down-set of a poset being a chart.  So the family lifts the *base's* cells: a 0-cell is a run (`runPtEquiv`), a 1-cell a generator of `p` making a germ step between two runs (`gen_action`), a 2-cell a relation of `p` holding there — and it moves when `p` does.  Functoriality is **strict**: a merge left-translates a germ step (`germStep_push`), so it is a map of charts (`runChartPush`) and `Polygraph.comapOver` carries it with the 2-cells untouched (`slicePush`); the strand count is *data* in a 0-cell rather than a proof, which is what makes `sliceRawFunctor : Ch Zbp ⥤ Polygraph` an honest functor.  `hP` is the whole obligation, and it is free — the 0-cells name their own slice objects (`sliceCellOver`) and pushing them is `Over.map` — so there is no condition on the base at all | `Concurrency/Presentation/SliceGerm.lean`, `Concurrency/Presentation/SliceInherit.lean`, `Concurrency/Presentation/GermWeakOrder.lean`, `Concurrency/Presentation/SliceRunSet.lean` |
...
- `WeakAction.lean` — `WeakDown X`, closure of a set of permutations under the right weak order,
  stated on `permLen` alone so that it is available below `WeakOrder`.  It is the one hypothesis a
  `GermChart` asks of its carrier.
...
- `Day.lean` — **`PolyShape` is promonoidal**.  It is not monoidal (`cell m n ⊗ cell m' n'` would be
  ...
  convolution needs.  The profunctor `pro c a b = Σ s : Split c, (s.fst ⟶ a) × (s.snd ⟶ b)` is a
...
- `Pi.lean` — the same tensor over a finite index, in a strictly associative model:
  `Polygraph.pi`, `Presents.pi`, and `piFunctor` with `piMap_id`/`piMap_comp` on the nose.  Cells
  pin their 0-cells by a `Shift`, never by a `Function.update` in the index; `exists_piNormalForm`
  sorts every word by index.  Strictness is all this model buys over `Monoidal.lean`'s associator.
...
- `LengthGraded.lean` — `Polygraph.LengthGraded`: no 2-cell relates two words of one length.  An
  interchange square does, so a length-graded polygraph receives no map out of a tensor with two
  factors carrying 1-cells.  `Hom` reflects the property, `comap` and `op` preserve it, and
  `Polygraph.pi` inherits it exactly when the index is a subsingleton.
...
- `SliceExchange.lean` — the assembly: the localized slice **is** the weak order wherever the runs
  ...
  *skeletal*.
- `SliceRunSet.lean` — the runs over `d` (`RunAt`, `RunAt.perm`) and their **down-set**:
  `runSet_of_le` iterates the exchange, `weakDown_runSet` is the closure, and `germStep_push` says a
  merge left-translates a germ step, so it leaves length-additivity alone.
- `GermWeakOrder.lean` — `GermChart n`: a set of permutations, named injectively, closed downwards.
  `dehornoy` presents its order by `p`'s germ — 1-cells the generators making a germ step, 2-cells
  `p`'s own relations — with `germProj_star_injective` the covering that makes words lift.
- `SliceGerm.lean` — the slice **is** a germ chart: `runGermChart d N`, `sliceLocEquiv d` reading the
  localized slice as its order backwards, `slicePoly` / `slicePresents` the coproduct over the strand
  counts, and `slicePush` / `sliceRawFunctor` / `p.fam` its strict functoriality in `d`.
  `germPresentsCube n` is the cube, with no `Option` anywhere in the construction.
- `SliceInherit.lean` — **the cell dictionary of the slice family**: `runPtEquiv` (a 0-cell is a
  ...
  `slicePoly_hP`, and hence `BraidPresentation.Br` / `.presentsBr`, `presentsChainsGarsideColimit` /
