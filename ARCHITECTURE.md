...
| **…and the charts they act on are the weak order** | `chartWeakEquiv n : ChartCat n ≌ WeakOrder n`.  A chart over the run *is* its crossing permutation (`crossOnesEquiv`), and a braid is defined at it exactly where it adds all of its own crossings — one length equation, `permLen (crossOnes x) + toAdd (posLen n β) = permLen (crossOnes y)` (`chartActionAt_eq_some_iff`), which by subadditivity of `permLen` forces both that `β` is a simple (`eq_posPerm_of_posLen`) and that the product is length-additive, i.e. exactly the right weak Bruhat order.  The `ᵐᵒᵖ`'s inverse is `matsuLift_op : matsuLift (op ∘ g) σ = op (matsuLift g σ⁻¹)`.  This is a second, independent route to the order that `locCubeWeakOrder` reaches from thinness | `Concurrency/Presentation/CubeChartWeakOrder.lean`, `Machinery/Braid/Matsumoto.lean` |
