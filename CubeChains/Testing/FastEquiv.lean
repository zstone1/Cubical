...
/-!
# Testing/FastEquiv — the fast model enumerates exactly the executions of `□n`

`FExec n` (`Testing/FastExec`) is a word plus a grouping into consecutive nonempty blocks; the
chain side (`Salvetti/ExecData`) presents an execution as `ExecData n`, a chain plus a run word.
`fexecChStarEquiv : FExec n ≃ Ch⋆ (□n)` bridges them: tagging each letter of the word with its
block index gives a weakly monotone label list, and monotonicity is exactly `WordCompat`.
