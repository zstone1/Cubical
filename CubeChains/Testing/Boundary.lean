...
proper face lies on it and the only cell of dimension `n` is the top one.  Hence `Ch⋆(∂□ⁿ)` is a real
...
/-- **The boundary `∂□ⁿ`** as a bi-pointed precubical set — the cube's own endpoints survive (they are
`0`-cells, and `0 < n`). -/
...
/-- **The inclusion `∂□ⁿ ↪ □ⁿ`** of bi-pointed sets — the subfunctor mono, endpoints preserved on the
nose. -/
