...
/-!
# Testing/HBar — the natural maps between powers of `H`

`H = symFree ⋙ symRestrict` carries the monad structure of `symFreeAdj`.  Its multiplication is
`(τ, σ, y) ↦ (σ * τ, y)` (`Hmul_app`) — the shear coordinate of `Testing/HTwo` — and it is the
...
`Testing/HTwo`'s `pattern`/`imageOf` **are** `SHom.sortPerm`/`sortFace` (`sortCheck`).  Three
