...
/-!
# Testing/HWedge — is the crossing permutation a function of the `Ch (Hbp □n)` wedge map?

A `Ch (Hbp K)` arrow's wedge map is the `Ch K` refinement **twisted by the coarse object's run**
(`twist`, `Salvetti/ChStarSym`): bead `j` of the coarse execution contributes the ordered partition
of its *run positions* cut out by the beads of the fine one — `hwedge`.  Untwisted (`cwedge`,
positions in the bead's sorted direction set) the datum is the plain `Ch (□n)` refinement and
forgets the run.
