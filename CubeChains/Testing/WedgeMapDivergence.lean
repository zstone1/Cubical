...
(`Chains/Refine.lean`), **not** of the wedge-map category `Ch K`
...
`Ch K`.  (The complementary fact — that in `Ch K` the objects
...
`equivWedgeCat : RefineObj ≌ Ch K` needs `NonSelfLinked ∧ AdmitsAltitude`,
