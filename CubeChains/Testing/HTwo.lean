import CubeChains.Testing.Merges
import CubeChains.Foundations.SymPresheaf

/-!
# Testing/HTwo — `Ch (H² K)` and its localization at the bead merges
...
`W` is the class whose wedge map is a canonical cut (`isMono`/`inW` of `Testing/Merges`), i.e.
