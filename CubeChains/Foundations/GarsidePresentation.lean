...
A `Star` at `x` makes `x` terminal in the wide subcategory `W`; dually a `Costar` at `y` makes `y`
initial.  Between them the **simples** `y ⟶ x` present the whole localization: `GarsideMonoid W y x`
has one relation per factorisation `y ⟶ b ⟶ x` — a simple times a simple is a simple — and one
declaring the `W`-simple to be the unit.  That last relation is not optional: without it every
generator may go to a single idempotent.
...
The mirror of `Star`: `Costar.step` is `IsInitial.to_comp` and `Costar.root` is
`IsInitial.to_self`. -/
...
/-- **A costar for `W` at `y`**: `y` is initial among the `W`-arrows, so every object receives
exactly one `W`-arrow from `y`. -/
abbrev Costar (y : C) :=
...
variable (T : Costar W y)

/-- The costar's arrow out of the basepoint. -/
def s (a : C) : y ⟶ a := (Limits.IsInitial.to T (WideSubcategory.mk a)).hom

theorem mem (a : C) : W (T.s a) := (Limits.IsInitial.to T (WideSubcategory.mk a)).property

/-- Travelling on by `W` changes nothing. -/
theorem step {a b : C} (w : a ⟶ b) (hw : W w) : T.s a ≫ w = T.s b := by
...
/-- The basepoint is its own costar — what makes the pad at `y` invisible. -/
...
Padding an arrow by the costar on the left and the star on the right makes it a simple without
changing its class, since both pads are `W`.  At the two ends the pads are identities
(`Star.root`, `Costar.root`), which is the other round trip. -/
...
def pad (S : Star W x) (T : Costar W y) {a b : C} (f : a ⟶ b) : y ⟶ x := T.s a ≫ f ≫ S.t b

theorem mem_pad (S : Star W x) (T : Costar W y) {a b : C} {w : a ⟶ b} (hw : W w) :
    W (pad S T w) :=
  W.comp_mem _ _ (T.mem a) (W.comp_mem _ _ hw (S.mem b))

/-- **The pads are invisible in `LocMonoid W`.** -/
theorem locOf_pad (S : Star W x) (T : Costar W y) {a b : C} (f : a ⟶ b) :
...
theorem garOf_pad_comp (S : Star W x) (T : Costar W y) {a b c : C} (f : a ⟶ b) (g : b ⟶ c) :
    garOf W (pad S T g) * garOf W (pad S T f) = garOf W (pad S T (f ≫ g)) := by
  simpa [pad] using garOf_comp (W := W) (T.s a ≫ f) (g ≫ S.t c) (T.mem b) (S.mem b)

/-- **Forward**: an arrow goes to the simple it pads to. -/
def locToGar (S : Star W x) (T : Costar W y) : LocMonoid W →* GarsideMonoid W y x :=
...
theorem garToLoc_comp_locToGar (S : Star W x) (T : Costar W y) :
    (garToLoc W y x).comp (locToGar S T) = MonoidHom.id (LocMonoid W) :=
  locMonoid_ext fun f => locOf_pad S T f

theorem locToGar_comp_garToLoc (S : Star W x) (T : Costar W y) :
  ...
  garsideMonoid_ext fun h => by
    ...
    simp only [pad, T.root, S.root, Category.id_comp, Category.comp_id]
...
def locEquivGarside (S : Star W x) (T : Costar W y) : LocMonoid W ≃* GarsideMonoid W y x where
  toFun := locToGar S T
...
/-- **The endomorphisms of the localization at the star are the interval's monoid.** -/
noncomputable def endEquivGarside (S : Star W x) (T : Costar W y) :
