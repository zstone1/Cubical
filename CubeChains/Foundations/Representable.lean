...
Forward map `ev` (value on the free top cell); inverse the `canonicalMap` built from
iterated faces, with `left_inv`/`right_inv` via `app_unique`/`app_topCell`.  Also
provides `coface`, `trueCount`, `canonicalMap_*`.
-/
