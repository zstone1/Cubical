...
theorem cube_axisLabel {N k : ℕ} (c : (BPSet.cube N).toPsh.cells k) (δ : Fin k) :
    StdCube.nones (toStar ((BPSet.cube N).toPsh.map (axisEdge δ).op c)) 0
      = StdCube.nones (toStar c) δ := by
  have hmap : (BPSet.cube N).toPsh.map (axisEdge δ).op c
      = ((BPSet.cube N).toPsh.cubeMap c).app (op (Box.ob 1)) (axisEdge δ) := by
...
theorem evLabel_comap_cube {N : ℕ} (φ : K.toPsh ⟶ (BPSet.cube N).toPsh) (a : ChainCat.Obj K)
    (i : Fin a.dims.length) (δ : Fin ((a.dims.get i) : ℕ)) :
    evLabel (EdgeLabelling.comap φ (cubeLabelling N)) ⟨a, ⟨i, δ⟩⟩
      = StdCube.nones (toStar (φ.app _ (beadCell a i))) δ := by
  change (cubeLabelling N).label (φ.app _ (K.toPsh.map (axisEdge δ).op (beadCell a i)))
    = StdCube.nones (toStar (φ.app _ (beadCell a i))) δ
...
noncomputable def pushChain {N : ℕ} (φ : K.toPsh ⟶ (BPSet.cube N).toPsh) (a : ChainCat.Obj K) :
    RefineObj ((a.map.hom ≫ φ).app (op (Box.ob 0)) (BPSet.serialWedge a.dims).init)
      ((a.map.hom ≫ φ).app (op (Box.ob 0)) (BPSet.serialWedge a.dims).final) where
...
theorem blockOf_pushChain {N : ℕ} (φ : K.toPsh ⟶ (BPSet.cube N).toPsh) (a : ChainCat.Obj K)
    (k : Fin (pushChain φ a).cubes.length) :
    blockOf (pushChain φ a) k
      = StdCube.noneSet (toStar (φ.app _
          (beadCell a (Fin.cast (wedgeToCubes_length a.dims (a.map.hom ≫ φ)) k)))).val := by
  have step1 : blockOf (pushChain φ a) k
      = StdCube.noneSet (toStar (yonedaEquiv (BPSet.serialWedge.ι a.dims
  ...
  rw [step1]
  have hXY : yonedaEquiv (BPSet.serialWedge.ι a.dims
...
theorem cubeEmbed_runInjective {N : ℕ} (φ : K.toPsh ⟶ (BPSet.cube N).toPsh) :
  ...
  have hmem : StdCube.nones (toStar (φ.app _ (beadCell a i))) δ
  ...
  have hi : i = i' := by
    ...
    exact Fin.ext hval
  subst hi
  exact congrArg (Sigma.mk i) ((StdCube.nones (toStar (φ.app _ (beadCell a i)))).injective heq2)
...
structure Sculpture (K : BPSet) where
  ...
  embed : K.toPsh ⟶ (BPSet.cube dim).toPsh
