...
/-!
# Machinery/Arrangement/BraidCovector — a canonical normal form for braid covectors

A covector `braidSign w` of the braid arrangement records only the *relative order* of the values
of `w`.  The dense rank `denseRank w` (see `Machinery/Arrangement/Braid.lean`) — the number of
distinct values of `w` strictly below `w i` — is the canonical representative: its values are
exactly `{0, 1, …, k-1}` where `k` is the number of distinct values.  This file packages that data
as an
