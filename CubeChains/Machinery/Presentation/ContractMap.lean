...
/-!
# Machinery/Presentation/ContractMap — a collapse is functorial in the polygraph

A map of polygraphs *reflecting* the collapsed family and taking each chosen merge to the chosen
merge carries one collapse to another.  The square below is `Map.pre_words`, and the two boundary
...
carries that, and nothing else here does.  The formal inverse needs no field of its own: a word of
picked letters is carried to a word of picked letters, so `invWord_hom` is `word_hom` reversed.
-/
...
variable {P Q R : Polygraph.{w, u', w₂}} {S : ∀ {a b : P.V}, P.Gen a b → Prop}
  ...
  {c : Collapse P S} {c' : Collapse Q T} {c'' : Collapse R U}
...
theorem words_nil (c : Collapse P S) (u : GenObj P.Gen) :
    c.words.map (Quiver.Path.nil : Quiver.Path u u) = Quiver.Path.nil :=
  Paths.lift_nil c.pre u

theorem words_cons (c : Collapse P S) {u v z : GenObj P.Gen} (R : Quiver.Path u v) (e : v ⟶ z) :
    c.words.map (R.cons e) = (c.words.map R).comp (c.cell e) :=
  Paths.lift_cons c.pre R e

/-- **Data carrying one collapse to another**: a map of polygraphs that reflects the collapsed
...
/-! ## The collapsed polygraph, on a map -/
...
/-- A conjugated non-collapsed letter is carried to the conjugate of its image. -/
...
/-- **The conjugation squares with the map, on a letter** — a collapsed letter becomes the empty
...
`backPre` conjugates a letter by the two chosen merges, so the square for `backSpelling` asks for
`word_hom` in the extension, and for its formal inverse — which the reversed word of inverses
supplies, with no second field. -/

/-- **The map of extensions.** -/
def invHom : invPoly P S ⟶ invPoly Q T :=
  invPolyMap S T m.hom fun e he => (m.mem_iff e).mpr he

theorem locWord_hom (x : P.V) :
    m.invHom.pre.mapPath (c.locWord x)
      = cellCongr Quiver.Path rfl (congrArg (fwdPre Q T).obj (congrArg Q.pt (m.rep_hom x)))
          (c'.locWord (m.obj x)) :=
  invPolyMap_mapPath_fwd_cellCongr S T m.hom (fun e he => (m.mem_iff e).mpr he)
    (c.word x) (c'.word (m.obj x))
    (congrArg Q.pt (m.rep_hom x)) (m.word_hom x)

/-- **…and the formal inverse of the merge to the formal inverse** — a word of picked letters
reverses to a word of their inverses, so this is `word_hom` read the other way. -/
theorem invWord_hom (x : P.V) :
    m.invHom.pre.mapPath (c.invWord x)
      = cellCongr Quiver.Path (congrArg (fwdPre Q T).obj (congrArg Q.pt (m.rep_hom x))) rfl
          (c'.invWord (m.obj x)) :=
  invPolyMap_mapPath_invWord_cellCongr S T m.hom (fun e he => (m.mem_iff e).mpr he)
    (c.word x) (c.word_all x)
    (c'.word (m.obj x)) (congrArg Q.pt (m.rep_hom x)) (m.word_hom x) (c'.word_all (m.obj x))

/-- **The conjugate of a letter, carried along.** -/
theorem pre_backWord {u v : GenObj P.Gen} (g : u ⟶ v) :
    m.invHom.pre.mapPath (c.backWord g)
      = cellCongr Quiver.Path (congrArg (fwdPre Q T).obj (congrArg Q.pt (m.rep_hom u.as)))
          (congrArg (fwdPre Q T).obj (congrArg Q.pt (m.rep_hom v.as)))
          (c'.backWord (m.hom.pre.map g)) := by
  refine Eq.trans (Prefunctor.mapPath_comp m.invHom.pre (c.invWord u.as)
    (((fwdPre P S).map g).toPath.comp (c.locWord v.as))) ?_
  refine Eq.trans (congrArg (Quiver.Path.comp (m.invHom.pre.mapPath (c.invWord u.as)))
    ((Prefunctor.mapPath_comp m.invHom.pre ((fwdPre P S).map g).toPath (c.locWord v.as)).trans
      (congrArg (fun t => Quiver.Path.comp t (m.invHom.pre.mapPath (c.locWord v.as)))
        (Prefunctor.mapPath_toPath m.invHom.pre ((fwdPre P S).map g))))) ?_
  refine Eq.trans (congrArg₂ Quiver.Path.comp (m.invWord_hom u.as)
    (congrArg (Quiver.Path.comp ((fwdPre Q T).map (m.hom.pre.map g)).toPath)
      (m.locWord_hom v.as))) ?_
  refine Eq.trans (congrArg (Quiver.Path.comp
      (cellCongr Quiver.Path (congrArg (fwdPre Q T).obj (congrArg Q.pt (m.rep_hom u.as))) rfl
        (c'.invWord (m.obj u.as))))
    (cellCongr_comp rfl rfl (congrArg (fwdPre Q T).obj (congrArg Q.pt (m.rep_hom v.as)))
      ((fwdPre Q T).map (m.hom.pre.map g)).toPath (c'.locWord (m.obj v.as)))) ?_
  exact cellCongr_comp (congrArg (fwdPre Q T).obj (congrArg Q.pt (m.rep_hom u.as))) rfl
    (congrArg (fwdPre Q T).obj (congrArg Q.pt (m.rep_hom v.as))) (c'.invWord (m.obj u.as)) _

/-- **…so a 1-cell of the collapse reads back to the 1-cell its image reads back to.** -/
theorem pre_backPre {X Y : GenObj c.Gen} (e : X ⟶ Y) :
    m.invHom.pre.mapPath (c.backPre.map e) = c'.backPre.map (m.pre.map e) := by
  refine Eq.trans (Prefunctor.mapPath_cellCongr m.invHom.pre
    (congrArg (invPoly P S).pt e.rep_dom) (congrArg (invPoly P S).pt e.rep_cod)
    (c.backWord (Polygraph.cell e.gen))) ?_
  refine Eq.trans (congrArg (cellCongr Quiver.Path
      (congrArg m.invHom.pre.obj (congrArg (invPoly P S).pt e.rep_dom))
      (congrArg m.invHom.pre.obj (congrArg (invPoly P S).pt e.rep_cod)))
...
theorem backSpelling_functor_map_quot {X Y : GenObj c.poly.Gen} (e : X ⟶ Y) :
    c'.backSpelling.functor.map (m.poly.functor.map (c.poly.quot.map e.toPath))
      = m.invHom.functor.map (c.backSpelling.functor.map (c.poly.quot.map e.toPath)) :=
  show (invPoly Q T).quot.map ((Paths.lift c'.backPre).map (m.pre.map e).toPath)
      = (invPoly Q T).quot.map (m.invHom.pre.mapPath ((Paths.lift c.backPre).map e.toPath)) from
  (congrArg (invPoly Q T).quot.map (Paths.lift_toPath c'.backPre (m.pre.map e))).trans
    ((congrArg (invPoly Q T).quot.map (m.pre_backPre e).symm).trans
      (congrArg (fun t => (invPoly Q T).quot.map (m.invHom.pre.mapPath t))
        (Paths.lift_toPath c.backPre e).symm))

/-- **Reading back is natural for a map of collapses** — on the nose, no coherence. -/
theorem backSpelling_functor_naturality :
    m.poly.functor ⋙ c'.backSpelling.functor = c.backSpelling.functor ⋙ m.invHom.functor :=
...
/-- The identity. -/
def id (c : Collapse P S) : Map c c where
  hom := 𝟙 P
  mem_iff _ := Iff.rfl
  rep_hom _ := rfl
...
/-- **A family of collapses along a functor into `Polygraph` is a functor** — the two laws are the
