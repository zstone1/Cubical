...
variable (X : Dᵒᵖ ⥤ Type w) {P : D ⥤ Polygraph.{w', u', w₂}}
...
theorem glueOnPt_map {s : GlueV X} (hs : s ∈ S) {d' : D} (f : d' ⟶ s.1)
    ...
    (glueOnPt X L S s hs ((P.map f).pre.obj a).as).1 = gluePt X L d' (X.map f.op s.2) a.as :=
...
theorem glueOnPt_span {s₁ s₂ : GlueV X} (h₁ : s₁ ∈ S) (h₂ : s₂ ∈ S) {d' : D}
    ...
    (a : GenObj (P.obj d').Gen) :
    glueOnPt X L S s₁ h₁ ((P.map f₁).pre.obj a).as
      = glueOnPt X L S s₂ h₂ ((P.map f₂).pre.obj a).as :=
...
inductive GlueOnEq (X : Dᵒᵖ ⥤ Type w) {P : D ⥤ Polygraph.{w', u', w₂}} (L : SliceLabels P)
    (S : Set (GlueV X)) :
  ...
  | span {s₁ s₂ : GlueV X} (h₁ : s₁ ∈ S) (h₂ : s₂ ∈ S) {d' : D}
      ...
      (hA : glueOnPt X L S s₁ h₁ ((P.map f₁).pre.obj a).as = A)
      (hB : glueOnPt X L S s₁ h₁ ((P.map f₁).pre.obj b).as = B) :
      GlueOnEq X L S ⟨s₁, h₁, _, _, (P.map f₁).pre.map g, hA, hB⟩
        ⟨s₂, h₂, _, _, (P.map f₂).pre.map g,
...
theorem glueOnPre_obj_span {s₁ s₂ : GlueV X} (h₁ : s₁ ∈ S) (h₂ : s₂ ∈ S) {d' : D}
    (f₁ : d' ⟶ s₁.1) (f₂ : d' ⟶ s₂.1) (hx : X.map f₁.op s₁.2 = X.map f₂.op s₂.2)
    (a : GenObj (P.obj d').Gen) :
    (glueOnPre X L S s₁ h₁).obj ((P.map f₁).pre.obj a)
      = (glueOnPre X L S s₂ h₂).obj ((P.map f₂).pre.obj a) :=
...
theorem glueOnPre_span_heq {s₁ s₂ : GlueV X} (h₁ : s₁ ∈ S) (h₂ : s₂ ∈ S) {d' : D}
    ...
    (glueOnPre X L S s₁ h₁).map ((P.map f₁).pre.map g)
      ≍ (glueOnPre X L S s₂ h₂).map ((P.map f₂).pre.map g) := by
  have key : ∀ {A B A' B' : GlueOnV X L S} (hA : A' = A) (hB : B' = B)
      (r₁ : GlueOnRaw X L S A B) (r₂ : GlueOnRaw X L S A' B'),
      GlueOnEq X L S r₁ ⟨r₂.s, r₂.hs, r₂.a, r₂.b, r₂.g, r₂.src.trans hA, r₂.tgt.trans hB⟩ →
      (Quot.mk _ r₁ : GlueOnGen X L S A B) ≍ (Quot.mk _ r₂ : GlueOnGen X L S A' B') := by
  ...
  exact key (glueOnPt_span X L S h₁ h₂ f₁ f₂ hx a).symm
    ...
    (GlueOnEq.span h₁ h₂ f₁ f₂ hx g rfl rfl)

/-- 2-cells: each copy's own; the overlaps are already taken on the 1-cells. -/
inductive GlueOnRel : GenObj (GlueOnGen X L S) → GenObj (GlueOnGen X L S) →
    Type (max u₁ w u' w' w₂)
  | copy (s : GlueV X) (hs : s ∈ S) {a b : GenObj (P.obj s.1).Gen} :
      (P.obj s.1).Rel a b →
      GlueOnRel ((glueOnPre X L S s hs).obj a) ((glueOnPre X L S s hs).obj b)

/-- A copy's 2-cell keeps its source, read in the copy. -/
def GlueOnRel.src : ∀ {v t : GenObj (GlueOnGen X L S)}, GlueOnRel X L S v t → Quiver.Path v t
  | _, _, .copy s hs α => (glueOnPre X L S s hs).mapPath ((P.obj s.1).src α)

/-- …and its target. -/
def GlueOnRel.tgt : ∀ {v t : GenObj (GlueOnGen X L S)}, GlueOnRel X L S v t → Quiver.Path v t
  | _, _, .copy s hs α => (glueOnPre X L S s hs).mapPath ((P.obj s.1).tgt α)

/-- **The glued polygraph over a generating set.** -/
def glueOn : Polygraph.{max u₁ w u' w', max u₁ w, max u₁ w u' w' w₂} where
  V := GlueOnV X L S
  Gen := GlueOnGen X L S
  Rel := GlueOnRel X L S
  src := GlueOnRel.src X L S
  tgt := GlueOnRel.tgt X L S

/-- The inclusion of the copy of `P s.1` at `s ∈ S`. -/
def glueOnIncl (s : GlueV X) (hs : s ∈ S) : Hom (P.obj s.1) (glueOn X L S) where
  pre := glueOnPre X L S s hs
  two α := GlueOnRel.copy s hs α
  src_two _ := rfl
  tgt_two _ := rfl

/-- **The span identification, as an equation between 1-cells** — `glueOnPre_span_heq`, conjugated
...
theorem glueOnPre_span {s₁ s₂ : GlueV X} (h₁ : s₁ ∈ S) (h₂ : s₂ ∈ S) {d' : D}
    (f₁ : d' ⟶ s₁.1) (f₂ : d' ⟶ s₂.1) (hx : X.map f₁.op s₁.2 = X.map f₂.op s₂.2)
    {a b : GenObj (P.obj d').Gen} (g : a ⟶ b) :
    (glueOn X L S).quot.map ((glueOnPre X L S s₁ h₁).map ((P.map f₁).pre.map g)).toPath
      = eqToHom (congrArg (glueOn X L S).quot.obj
            (glueOnPre_obj_span X L S h₁ h₂ f₁ f₂ hx a)) ≫
        (glueOn X L S).quot.map ((glueOnPre X L S s₂ h₂).map ((P.map f₂).pre.map g)).toPath ≫
        eqToHom (congrArg (glueOn X L S).quot.obj
            (glueOnPre_obj_span X L S h₁ h₂ f₁ f₂ hx b)).symm :=
  (conj_eqToHom_iff_heq' _ _ _ _).2
    (quot_map_heq (glueOnPre_obj_span X L S h₁ h₂ f₁ f₂ hx a)
      (glueOnPre_obj_span X L S h₁ h₂ f₁ f₂ hx b)
      (glueOnPre_span_heq X L S h₁ h₂ f₁ f₂ hx g))
...
theorem glueOnAt_of_eq {A : GlueOnV X L S} {s : GlueV X} {hs : s ∈ S} {a : (P.obj s.1).V}
    ...
    (glueSliceEval X W s.1 s.2).obj ((p s.1).at' ⟨a⟩) = glueOnAt X L S W ⟨A⟩ :=
  (glueOnAt_glueOnPt X L S W p hL s hs a).trans
    (congrArg (fun v => glueOnAt X L S W ⟨v⟩) h)
...
noncomputable def glueOnRawArrow {A B : GlueOnV X L S} (r : GlueOnRaw X L S A B) :
    glueOnAt X L S W ⟨A⟩ ⟶ glueOnAt X L S W ⟨B⟩ :=
  eqToHom (glueOnAt_of_eq X L S W p hL r.src).symm ≫
    (glueSliceEval X W r.s.1 r.s.2).map ((p r.s.1).arrow r.g) ≫
    eqToHom (glueOnAt_of_eq X L S W p hL r.tgt)
...
theorem glueOnRawArrow_span {A B : GlueOnV X L S} {r r' : GlueOnRaw X L S A B}
    (h : GlueOnEq X L S r r') :
    glueOnRawArrow X L S W p hL r = glueOnRawArrow X L S W p hL r' := by
  cases h with
...
noncomputable def glueOnArrow :
    ∀ {v t : GenObj (GlueOnGen X L S)}, (v ⟶ t) →
      (glueOnAt X L S W v ⟶ glueOnAt X L S W t) :=
  fun {v t} e => Quot.liftOn (e : GlueOnGen X L S v.as t.as)
    (glueOnRawArrow X L S W p hL) fun _ _ h => glueOnRawArrow_span X L S W p hL hP h
...
noncomputable def glueOnEval : GenObj (GlueOnGen X L S) ⥤q
  ...
  map := glueOnArrow X L S W p hL hP

include hL in
theorem glueOnEval_map_glueOnPre (s : GlueV X) (hs : s ∈ S)
    {a b : GenObj (P.obj s.1).Gen} (e : a ⟶ b) :
    (glueOnEval X L S W p hL hP).map ((glueOnPre X L S s hs).map e) =
...
theorem glueOnEval_mapPath (s : GlueV X) (hs : s ∈ S) {a b : GenObj (P.obj s.1).Gen}
    (u : Quiver.Path a b) :
    (Paths.lift (glueOnEval X L S W p hL hP)).map ((glueOnPre X L S s hs).mapPath u) =
      eqToHom (glueOnAt_glueOnPt X L S W p hL s hs a.as).symm ≫
        (glueSliceEval X W s.1 s.2).map ((p s.1).eval.map u) ≫
        eqToHom (glueOnAt_glueOnPt X L S W p hL s hs b.as) := by
  induction u with
  | nil =>
      rw [show (Paths.lift (glueOnEval X L S W p hL hP)).map
            ((glueOnPre X L S s hs).mapPath (Quiver.Path.nil : Quiver.Path a a)) = 𝟙 _ from
          (Paths.lift (glueOnEval X L S W p hL hP)).map_id _,
...
theorem glueOn_sound {v t : GenObj (GlueOnGen X L S)} (α : (glueOn X L S).Rel v t) :
    (Paths.lift (glueOnEval X L S W p hL hP)).map ((glueOn X L S).src α)
      = (Paths.lift (glueOnEval X L S W p hL hP)).map ((glueOn X L S).tgt α) := by
  cases α with
  | copy s hs hr =>
      change (Paths.lift (glueOnEval X L S W p hL hP)).map
            ((glueOnPre X L S s hs).mapPath ((P.obj s.1).src hr))
          = (Paths.lift (glueOnEval X L S W p hL hP)).map
            ((glueOnPre X L S s hs).mapPath ((P.obj s.1).tgt hr))
...
noncomputable def glueOnDesc : (glueOn X L S).presented ⥤
    (W.inverseImage (CategoryOfElements.π X).leftOp).Localization :=
  (glueOn X L S).desc (glueOnEval X L S W p hL hP) (glueOn_sound X L S W p hL hP)
...
theorem glueOnIncl_desc (s : GlueV X) (hs : s ∈ S) :
  ...
  rw [← Functor.assoc, Hom.quot_comp_functor, Functor.assoc,
    show (glueOn X L S).quot ⋙ glueOnDesc X L S W p hL hP
        = Paths.lift (glueOnEval X L S W p hL hP) from
      (glueOn X L S).quot_comp_desc _ (glueOn_sound X L S W p hL hP), ← Functor.assoc]
  refine Functor.ext (fun a => (glueOnAt_glueOnPt X L S W p hL s hs a.as).symm) ?_
  intro a b u
  exact glueOnEval_mapPath X L S W p hL hP s hs u
...
theorem gluePt_sliceBase {s : GlueV X} {c : (X.Elements)ᵒᵖ} (k : c ⟶ elt X s) :
    gluePt X L s.1 s.2 ((P.map ((CategoryOfElements.π X).leftOp.map k)).pre.obj
...
noncomputable def sliceBaseOver {s : GlueV X} {c : (X.Elements)ᵒᵖ} (k : c ⟶ elt X s) :
    (p s.1).at' ⟨((P.map ((CategoryOfElements.π X).leftOp.map k)).pre.obj
        ⟨sliceBase W p ((CategoryOfElements.π X).leftOp.obj c)⟩).as⟩
      ≅ (W.over (X := s.1)).Q.obj
...
theorem exists_wordIso_glueOnBase (hgen : Generating X S) (v : GlueOnV X L S) :
  ...
  have hkey : (W.over (X := s.1)).Q.obj
    ...
    (hL s.1 a).symm
  set ι : (p s.1).at' ⟨((P.map ((CategoryOfElements.π X).leftOp.map kA)).pre.obj
...
theorem spanIncl_eq {d' : D} {s₁ s₂ : GlueV X} (h₁ : s₁ ∈ S) (h₂ : s₂ ∈ S)
  ...
  have hword : ∀ {s : GlueV X} (hs : s ∈ S) (f : d' ⟶ s.1)
      ...
      ((P.map f).words ⋙ (glueOnIncl X L S s hs).words ⋙ (glueOn X L S).quot).map
          (Quiver.Hom.toPath e)
        = (glueOn X L S).quot.map
            ((glueOnPre X L S s hs).map ((P.map f).pre.map e)).toPath := by
  ...
  exact glueOnPre_span X L S h₁ h₂ f₁ f₂ hx e
    ...
    (glueOnRetractPre X L S W p hL hthin R hgen c).obj (Over.mk (𝟙 c))
      = (glueOn X L S).quot.obj
          ⟨glueOnPt X L S (genPt X S hgen c) (genPt_mem X S hgen c)
            (((P.map ((CategoryOfElements.π X).leftOp.map (genLeg X S hgen c))).pre.obj
...
theorem glueOnUnit (hgen : Generating X S) :
  ...
  rw [← Functor.assoc, show (glueOn X L S).quot ⋙ glueOnDesc X L S W p hL hP
      = Paths.lift (glueOnEval X L S W p hL hP) from
    (glueOn X L S).quot_comp_desc _ (glueOn_sound X L S W p hL hP), Functor.comp_id]
  ...
  · rintro ⟨a⟩ ⟨b⟩ ⟨s, hs, a', b', g, hsrc, htgt⟩
    ...
    refine Eq.trans (congrArg (glueOnRetractDesc X L S W p hL hthin R hgen).map
      ((Paths.lift_toPath _ _).trans (glueOnEval_map_glueOnPre X L S W p hL hP s hs g))) ?_
