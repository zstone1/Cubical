...
/-!
# Machinery/Braid/PermWord — the Artin word emitter
...
`Machinery/Braid/Kernel`).  `schreierWordZ σ j` emits its signed braid word, and `wordZToBraid`
reads it back as that pure braid. -/
