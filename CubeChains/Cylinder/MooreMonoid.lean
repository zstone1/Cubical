...
* **Geometry (the Moore cylinder realising the product).**  A section-cylinder
  `c : SecCyl K` induces `c.toPointedObj` (`Cylinder/CylinderRefine.lean`).  A finite
  composable *staircase* of such cylinders — a `List (SecCyl K)` — induces the monoid product
  `mooreInduced m := (m.map SecCyl.toPointedObj).prod`.  Crucially this staircase is realised
...
* **THE SUBMONOID** `mooreSubmonoid K` : `Set.range mooreInduced` is a
  ...
  `Submonoid.closure (Set.range SecCyl.toPointedObj)`, with unit `1` realised by the length-`0`
...
* **The geometric certificate** `mul_eq_mooreInduced_pair` + the underlying geometric facts
  ...
  `MooreCyl.mooreComposeIdRight`): the product `SecCyl.toPointedObj c * SecCyl.toPointedObj c'` is
  realised by the *single* geometric Moore cylinder `mooreCompose (pieceCyl c) (pieceCyl c')` of
  length `2`
...
`ofCylMap`, the descent lemma), `Cylinder/CylinderRefine` (`SecCyl.toPointedObj`, `DPathGrpdR`).
...
strict section-cylinders' endofunctors, with the consecutive geometric `mooreCompose`
...
/-! ## 1. A section-cylinder as a geometric length-`1` Moore cylinder

A `SecCyl K` object carries (in its underlying cylinder `obj`) a classifying map
`cyl : src.toPsh ⟶ PathOb K.toPsh`, i.e. exactly the data of a `CylMap K.toPsh =
Over (PathOb K.toPsh)`.  Through `MooreCyl.ofCylMap` it is a geometric length-`1` Moore cylinder of
`K.toPsh`. -/

/-- The underlying ordinary cylinder `CylMap K.toPsh` of a section-cylinder: its classifying map
`obj.cyl` viewed as an over-object of the path object `PathOb K.toPsh`. -/
noncomputable def toCylMap (c : SecCyl K) : CylMap K.toPsh := Over.mk c.obj.cyl

/-- The **geometric length-`1` Moore cylinder** of a section-cylinder: `ofCylMap` of its underlying
ordinary cylinder.  This is the genuine geometric homotopy `src ⊗ □¹ ⟶ K` underlying the
strict cylinder. -/
noncomputable def pieceCyl (c : SecCyl K) : MooreCyl K.toPsh :=
  MooreCyl.ofCylMap (toCylMap c)

@[simp] theorem pieceCyl_n (c : SecCyl K) : (pieceCyl c).n = 1 := rfl

end CylMapR

/-! ## 2. The induced pointed endofunctor of a Moore staircase

A **Moore staircase** over `K` is a finite list of composable section-cylinders.  Its
induced pointed endofunctor is the monoid product (in list order) of the components'
`SecCyl.toPointedObj` — the staircase is realised geometrically by iterating `mooreCompose` (§3),
but the induced endofunctor is read off algebraically from the strict pieces (no new sweep). -/

namespace MooreCyl

/-- **The induced pointed endofunctor of a Moore staircase** `m : List (SecCyl K)`: the monoid
product (in list order) of the components' `SecCyl.toPointedObj`.  The empty staircase —
geometrically the length-`0` unit `mooreId` — induces `1`; a singleton induces the strict
cylinder's endofunctor;
a cons multiplies on the left. -/
noncomputable def mooreInduced (m : List (SecCyl K)) :
    PointedEndofunctor (DPathGrpdR K) :=
  (m.map SecCyl.toPointedObj).prod
...
@[simp] theorem mooreInduced_nil :
    mooreInduced ([] : List (SecCyl K)) = 1 := by
  simp [mooreInduced]

/-- **A length-`1` Moore staircase is the strict component.**  `mooreInduced [c] =
SecCyl.toPointedObj c` — geometrically `pieceCyl c`, the genuine length-`1` Moore cylinder. -/
@[simp] theorem mooreInduced_singleton (c : SecCyl K) :
    mooreInduced [c] = SecCyl.toPointedObj c := by
  simp [mooreInduced]

/-- A cons unfolds as a product:
`mooreInduced (c :: m) = SecCyl.toPointedObj c * mooreInduced m`. -/
@[simp] theorem mooreInduced_cons (c : SecCyl K) (m : List (SecCyl K)) :
    mooreInduced (c :: m) = SecCyl.toPointedObj c * mooreInduced m := by
  simp [mooreInduced]
...
transport for free). -/
@[simp] theorem mooreInduced_concat (m m' : List (SecCyl K)) :
...
/-- The **geometric Moore cylinder realising a staircase** `m : List (SecCyl K)`: iterate the
...
noncomputable def geomList : List (SecCyl K) → MooreCyl K.toPsh
  | [] => mooreId K.toPsh
  | c :: m => mooreCompose (CylMapR.pieceCyl c) (geomList m)

@[simp] theorem geomList_nil : geomList ([] : List (SecCyl K)) = mooreId K.toPsh := rfl

@[simp] theorem geomList_cons (c : SecCyl K) (m : List (SecCyl K)) :
...
@[simp] theorem geomList_n (m : List (SecCyl K)) : (geomList m).n = m.length := by
  induction m with
  | nil => rfl
  | cons c m ih =>
...
theorem geomList_cons_isMooreCompose (c : SecCyl K) (m : List (SecCyl K)) :
    geomList (c :: m) = mooreCompose (CylMapR.pieceCyl c) (geomList m) := rfl

/-! ## 3. The geometric certificate — `mooreCompose`, not a free append

The product `SecCyl.toPointedObj c * SecCyl.toPointedObj c'` is realised by the *single geometric*
Moore cylinder `mooreCompose (pieceCyl c) (pieceCyl c')`: the span-pullback gluing of the two strict
...
/-- **Product = length-`2` staircase, algebraically.**  `SecCyl.toPointedObj c *
SecCyl.toPointedObj c' = mooreInduced [c, c']` — the product of two cylinder endofunctors IS the
induced endofunctor of the
length-`2` Moore staircase `[c, c']`. -/
theorem mul_eq_mooreInduced_pair (c c' : SecCyl K) :
    SecCyl.toPointedObj c * SecCyl.toPointedObj c'
      = mooreInduced [c, c'] := by
...
For two section-cylinders `c, c'`, the geometric Moore composition of their length-`1`
...
theorem mooreCompose_pieceCyl_glue (c c' : SecCyl K) :
...
theorem mooreId_unit_geometric (m : List (SecCyl K)) (hm : m = []) :
    mooreInduced m = 1 ∧
...
@[simp] theorem mem_mooreSubmonoid {A : PointedEndofunctor (DPathGrpdR K)} :
    A ∈ mooreSubmonoid K ↔ ∃ m : List (SecCyl K), mooreInduced m = A :=
  Iff.rfl

/-- **The strict cylinder image is contained in `mooreSubmonoid`** — via the singleton (length-`1`)
Moore staircases (`mooreInduced [c] = SecCyl.toPointedObj c`).  The Moore submonoid contains every
strict cylinder endofunctor. -/
theorem range_cylToPointedObj_subset_mooreSubmonoid :
    Set.range (SecCyl.toPointedObj (K := K)) ⊆ ↑(mooreSubmonoid K) := by
  rintro _ ⟨c, rfl⟩
  exact ⟨[c], mooreInduced_singleton c⟩

/-- **`mooreSubmonoid K = Submonoid.closure (Set.range SecCyl.toPointedObj)`, on the nose.**

* `⊆` : every `mooreInduced m` is a finite product of `SecCyl.toPointedObj c`'s (a `List.prod` over
  the mapped staircase), each a generator, so it lies in the closure (`list_prod_mem`);
...
theorem mooreSubmonoid_eq_closure :
    mooreSubmonoid K = Submonoid.closure (Set.range (SecCyl.toPointedObj (K := K))) := by
  apply le_antisymm
  · rintro _ ⟨m, rfl⟩
    rw [mooreInduced]
    apply list_prod_mem
...
SecCyl.toPointedObj)` as plain sets — the `Submonoid`-coe of the previous theorem. -/
theorem range_mooreInduced_eq_closure :
    Set.range (mooreInduced (K := K))
      = (Submonoid.closure (Set.range (SecCyl.toPointedObj (K := K))) : Set _) :=
