...
`signAt` (`Arrangements/Braid.lean`), which supplies the sign flip when the relabeled pair comes
out inverted.  It is equivariant with `braidSign` (`reorient_braidSign`), a left `MulAction`, and
a COM-automorphism of `braidCOM n` (preserves covectors, the face order, and topes).
-/
