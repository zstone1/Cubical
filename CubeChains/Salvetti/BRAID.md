...
coend transports this to presheaves in closed form (`Foundations/GeoTensor`, `GeoTensor/BP`),
...
`Lines K : (Ch K)ᵒᵖ ⥤ Type` is the run presheaf (`Salvetti/Runs`); `Ch⋆ K` its category of
...
pair of strands crosses twice (`permOf_noDoubleCross`, `Salvetti/EventBraid`). That is the entire
...
names, while `Zbp`'s events have none — and `Ch⋆ Zbp ≌ RunWedge` (`Salvetti/RunWedgeZ`) grades to
...
the braid arrangement `braidCOM n` (`Arrangements/Braid`), locally `∏ᵢ F(ℂ, dᵢ)`. Configuration
...
of this in the tree is `Sal(braidCOM n) ≌ Ch⋆(□ⁿ)` (`Salvetti/SalExec`): the Salvetti complex is
