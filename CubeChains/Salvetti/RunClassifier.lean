...
import CubeChains.Salvetti.SymOverRun
import CubeChains.Salvetti.SymRun

/-!
# Salvetti/RunClassifier — the run object, and why `Hbp` is not a product
...
discrete fibrations (`Chains/ElementsFibration`) with fibres `wedgeHoms`; the tower is *not* a
product, by `not_desym_natural`. -/
