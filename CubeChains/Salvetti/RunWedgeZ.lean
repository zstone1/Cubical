...
/-!
# Salvetti/RunWedgeZ — `RunWedge` is the executions of the terminal precubical set

`Zbp` (`Foundations/Terminal`) is terminal in `BPSet`: a wedge maps into it uniquely, so a chain of
`Zbp` *is* just its wedge and a run of `Zbp` is just a run.  Hence `proj Zbp : Ch⋆ Zbp ⥤ RunWedge`
is an equivalence — the coordinate labelling that makes `proj □n` non-full collapses precisely
because `Hom(-, Zbp)` is a singleton.  So `RunWedge = Ch⋆ Zbp` and the braid on `RunWedge` is `Conc`
at the terminal object: the full braid group, not just the pure part.
