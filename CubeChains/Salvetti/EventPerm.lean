...
The lexicographic flattening `pos` (`Chains/CoordFunctor`) orders the beads; `chainBead_refine` is
the cross-bead half of no-double-crossing, and is all of it that survives run-freely — the
within-bead half genuinely needs the run order (`Salvetti/EventBraid`).
-/
