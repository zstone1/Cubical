...
built from `wedgeDesc`/`wedgeToCubes` (`Precubical/Chains/WedgeMap.lean`) and the chain bridge
`isCubeChain`/`ofIsCubeChain` (`Precubical/Chains/Basic.lean`), lifted to an equivalence of
