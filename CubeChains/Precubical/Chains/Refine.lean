...
A *refinement* `x ⟶ y` replaces each cube of the coarser chain `y` by a sub-chain of its faces —
Ziemiański's subdivision relation on `Ch(K)`.  It is a monotone reindexing of `x`-cubes into
`y`-cubes **together with the inclusion data**: an explicit `Box` morphism pulling the `y`-cube
back to the `x`-cube.

Carrying that inclusion as *data* rather than as the `Prop` that a face relation holds is what
makes the forward functor to wedge maps definable without rigidity assumptions on `K`.
-/
