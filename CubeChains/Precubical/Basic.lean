...
theorem face_face' (K : PrecubicalSet.{u}) {n : ℕ} (ε η : Bool) {i j : Fin (n + 1)}
    (h : i ≤ j) (c : K.cells (n + 2)) :
...
all face maps. -/
@[ext]
structure Hom (K L : PrecubicalSet.{u}) where
...
variable {K L M : PrecubicalSet.{u}}

/-- The identity morphism. -/
protected def id (K : PrecubicalSet.{u}) : Hom K K where
  app _ c := c
  app_face _ _ _ := rfl
...
instance : Category PrecubicalSet.{u} where
  Hom K L := Hom K L
  id K := Hom.id K
  comp f g := Hom.comp f g

@[simp]
theorem id_app (K : PrecubicalSet.{u}) (n : ℕ) (c : K.cells n) :
    Hom.app (𝟙 K) n c = c := rfl

@[simp]
theorem comp_app {K L M : PrecubicalSet.{u}} (f : K ⟶ L) (g : L ⟶ M) (n : ℕ)
...
@[ext]
theorem hom_ext {K L : PrecubicalSet.{u}} {f g : K ⟶ L}
...
def vertex (K : PrecubicalSet.{u}) (ε : Bool) : ∀ {n : ℕ}, K.cells n → K.cells 0
  | 0,     c => c
  | _ + 1, c => vertex K ε (K.face ε 0 c)

@[simp] theorem vertex_zero (K : PrecubicalSet.{u}) (ε : Bool) (c : K.cells 0) :
    K.vertex ε c = c := rfl

theorem vertex_succ (K : PrecubicalSet.{u}) (ε : Bool) {n : ℕ} (c : K.cells (n + 1)) :
...
theorem vertex_face (K : PrecubicalSet.{u}) (ε : Bool) :
...
def vertex₀ (K : PrecubicalSet.{u}) {n : ℕ} (c : K.cells n) : K.cells 0 :=
  K.vertex false c
...
@[simp] theorem vertex₀_zero (K : PrecubicalSet.{u}) (c : K.cells 0) :
    K.vertex₀ c = c := rfl
@[simp] theorem vertex₁_zero (K : PrecubicalSet.{u}) (c : K.cells 0) :
    K.vertex₁ c = c := rfl

/-- Taking a source face commutes with the source vertex. -/
theorem vertex₀_face (K : PrecubicalSet.{u}) {n : ℕ} (i : Fin (n + 1))
...
theorem map_vertex {K L : PrecubicalSet.{u}} (f : Hom K L) (ε : Bool) :
...
theorem map_vertex₀ {K L : PrecubicalSet.{u}} (f : Hom K L) {n : ℕ} (c : K.cells n) :
