...
obeying the precubical identity, plus the `Category` instance.  The topos `Boxᵒᵖ ⥤ Type` is the
model everything else is built on; `Precubical/Basic/Nerve` compares the two.

Convention: `ε : Bool` with `false = d⁰` (source) and `true = d¹` (target); the precubical
identity mirrors mathlib's `SimplicialObject.δ_comp_δ`.
-/
