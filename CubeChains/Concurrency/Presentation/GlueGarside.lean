...
/-!
# Concurrency/Presentation/GlueGarside — the Garside generators, and what they remember
...
`Br germBP (Hbp □ⁿ)` agrees on the 0-cells (`glueRunV_bijective`) and not on the 1-cells: a simple
is crossed in the one-bead chart, and there the run it was crossed above is remembered
(`GlueChart`), so a simple that mixes all `n` events names **two** 1-cells between one pair of
...
chart to. -/
theorem glueV_topRunAt (p : BraidPresentation) (K : BPSet) {n : ℕ}
    (X : ⋁(topDims n) ⟶ K) (σ : Perm (Fin n)) :
    glueV K p.fam (op ⟨op (zObj (topDims n)), X⟩) (p.runPt (topRunAt n σ))
      = p.glueRunV K (((onesTopEquiv n).symm σ).φ ≫ X) :=
  (congrArg (glueV K p.fam (op ⟨op (zObj (topDims n)), X⟩))
      (p.famV_runPt ((onesTopEquiv n).symm σ) (runAtSelf n)).symm).trans
    (glueV_leg K p.fam (eltLeg K ((onesTopEquiv n).symm σ) X) (p.runPt (runAtSelf n)))
...
noncomputable def germTopRaw (K : BPSet) {n : ℕ} (X : ⋁(topDims n) ⟶ K) (ρ σ : Perm (Fin n))
    (h : permLen ρ + permLen σ = permLen (ρ * σ)) :
    glueV K germBP.fam (op ⟨op (zObj (topDims n)), X⟩) (germBP.runPt (topRunAt n (ρ * σ)))
      ⟶ glueV K germBP.fam (op ⟨op (zObj (topDims n)), X⟩) (germBP.runPt (topRunAt n ρ)) :=
  glueE K germBP.fam (op ⟨op (zObj (topDims n)), X⟩)
...
noncomputable def germTopCell (K : BPSet) {n : ℕ} (X : ⋁(topDims n) ⟶ K) (ρ σ : Perm (Fin n))
    (h : permLen ρ + permLen σ = permLen (ρ * σ)) :
    germBP.glueRunV K (((onesTopEquiv n).symm (ρ * σ)).φ ≫ X)
      ⟶ germBP.glueRunV K (((onesTopEquiv n).symm ρ).φ ≫ X) :=
  Quiver.homOfEq (germTopRaw K X ρ σ h)
    (glueV_topRunAt germBP K X (ρ * σ)) (glueV_topRunAt germBP K X ρ)
...
theorem sepCells_germTopRaw (K : BPSet) {n : ℕ} (X : ⋁(topDims n) ⟶ K) (ρ σ : Perm (Fin n))
  ...
  rw [germTopRaw, sepCells_glueE, sepVal, dif_pos hc]
  simp only [hA, hB]
...
theorem arrow_glueE_comp (p : BraidPresentation) (K : BPSet) (c : ((wedgeHoms K).Elements)ᵒᵖ)
    ...
    (p.presentsBr K).arrow (glueE K p.fam c g) ≫ (p.presentsBr K).arrow (glueE K p.fam c g')
      = (p.presentsBr K).arrow (glueE K p.fam c g'') := by
  ...
  rw [p.arrow_glueE K c a b g, p.arrow_glueE K c b e g', p.arrow_glueE K c a e g'']
  exact conj_comp_of_eq
    (glueSliceEval (wedgeHoms K) (W Zbp) (eltBase (wedgeHoms K) c) c.unop.2
      ⋙ (locEquivElements K).inverse)
    (p.at_glueV K c a) (p.at_glueV K c b) (p.at_glueV K c e) _ _ _ hslice
...
theorem chBraid_germTopRaw (ρ σ : Perm (Fin n))
  ...
  have hcomp := arrow_glueE_comp germBP (Hbp.obj (□n)) c
    (germBP.runGen σ (action_topRunAt_mul ρ σ h))
...
A 0-cell of `Br p K` is a run's (`exists_glueRunV`), and at the decorated cube distinct runs give
...
theorem glueRunV_injective (p : BraidPresentation) (n : ℕ) :
    Function.Injective (p.glueRunV (Hbp.obj (□n)) (n := n)) := fun _ _ h =>
  eq_of_locIso_runCh ((p.glueRunIso (Hbp.obj (□n)) _).symm
    ≪≫ eqToIso (congrArg (p.presentsBr (Hbp.obj (□n))).at' h)
    ≪≫ p.glueRunIso (Hbp.obj (□n)) _)

theorem glueRunV_surjective (p : BraidPresentation) (n : ℕ) :
    Function.Surjective (p.glueRunV (Hbp.obj (□n)) (n := n)) := by
  intro A
  obtain ⟨c, w, hw⟩ := Polygraph.exists_colimit_ι_obj
    (elementsPoly (wedgeHoms (Hbp.obj (□n))) p.fam) A
  obtain ⟨z, hz⟩ := p.exists_glueRunV (Hbp.obj (□n)) c (hbpCubeStrands c.unop.2) w.as
  exact ⟨z, hz.symm.trans hw⟩

/-- **The 0-cells of `Br p (Hbp □ⁿ)` are the runs**, whatever the base presentation. -/
theorem glueRunV_bijective (p : BraidPresentation) (n : ℕ) :
    Function.Bijective (p.glueRunV (Hbp.obj (□n)) (n := n)) :=
  ⟨glueRunV_injective p n, glueRunV_surjective p n⟩
...
noncomputable def germObEquiv (n : ℕ) :
  ...
  (germActionV n).trans
    ((runFibreEquiv n).symm.trans (Equiv.ofBijective _ (glueRunV_bijective germBP n)))
...
noncomputable def wSrc : GenObj (germBP.Br (Hbp.obj (□3))).Gen :=
  germBP.glueRunV (Hbp.obj (□3)) ((hFibre 3).map (runLoop 3 (swap3 * rot3)) wRun)

/-- …and the one they run to. -/
noncomputable def wTgt : GenObj (germBP.Br (Hbp.obj (□3))).Gen :=
  germBP.glueRunV (Hbp.obj (□3)) wRun'
...
noncomputable def straightCell : wSrc ⟶ wTgt :=
  Quiver.homOfEq (germTopCell (Hbp.obj (□3)) (topWitness wRun') 1 rot3 addLen_one_rot3)
    (congrArg (germBP.glueRunV (Hbp.obj (□3)))
...
noncomputable def crossedCell : wSrc ⟶ wTgt :=
  Quiver.homOfEq (germTopCell (Hbp.obj (□3)) (topWitness wRun) swap3 rot3 addLen_swap3_rot3)
    (congrArg (germBP.glueRunV (Hbp.obj (□3))) (onesTop_topWitness (swap3 * rot3) wRun))
    (congrArg (germBP.glueRunV (Hbp.obj (□3))) (onesTop_topWitness swap3 wRun))
...
(`glueV_topLeg`) and the two cells are parallel with nothing to construct.  So `brZMap` is not a
...
noncomputable def straightLoopZ :
    germBP.glueRunV Zbp (zRun 3) ⟶ germBP.glueRunV Zbp (zRun 3) :=
  Quiver.homOfEq (germTopRaw (n := 3) Zbp (zObj (topDims 3)).map 1 rot3 addLen_one_rot3)
    (germBP.glueV_topLeg (N := 3) (1 * rot3)) (germBP.glueV_topLeg (N := 3) 1)
...
noncomputable def crossedLoopZ :
  ...
  Quiver.homOfEq (germTopRaw (n := 3) Zbp (zObj (topDims 3)).map swap3 rot3 addLen_swap3_rot3)
    (germBP.glueV_topLeg (N := 3) (swap3 * rot3)) (germBP.glueV_topLeg (N := 3) swap3)
