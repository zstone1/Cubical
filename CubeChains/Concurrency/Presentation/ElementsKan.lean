...
/-!
# Concurrency/Presentation/ElementsKan — the elements of a Kan extension are not the localization

`elementsKan α : ∫F ⥤ ∫G` compares the elements of a presheaf with those of an extension of it
along `L`.  It is not a localization.  At a representable `F = よc` — where `∫F` is the slice `C/c`
and `∫(Lan よc)` is `D/(L c)` — essential surjectivity says every arrow into `L c` is an arrow of
`C` up to isomorphism, a calculus-of-fractions demand.  `Ch Zbp` fails it at the square: the atom
loop `σ` of `PosBraid 2` is an endomorphism of `[2]` that no refinement performs, because a
refinement of two strands crosses at most once.  What sees this is `crossGrading`, the Coxeter
length of `crossPerm` — additive, and vanishing exactly on `W`, so unlike `codim` it survives the
localization.
-/
...
/-- **The square's loop**: the crossed staircase read over the straight one.  It is the generator
`σ` of `PosBraid 2` — the two-strand case of `LocPresentation`'s `atomLoop`, spelled here because
that file is far above this one — and it is not invertible, so `Ch Zbp[W⁻¹]` has arrows that no
single refinement performs. -/
noncomputable def sqLoop : (W Zbp).Q.obj (zObj [2]) ⟶ (W Zbp).Q.obj (zObj [2]) :=
  inv ((W Zbp).Q.map sqMerge) ≫ (W Zbp).Q.map sqAtom

theorem crossGradingLoc_sqLoop :
    (crossGradingLoc Zbp).codim sqLoop = (crossGrading Zbp).codim sqAtom := by
  rw [sqLoop, (crossGradingLoc Zbp).codim_comp,
...
theorem not_sliceEssSurj_sq : ¬ SliceEssSurj (W Zbp).Q (zObj [2]) := by
  intro hs
  obtain ⟨a, v, u, hv, hvu⟩ := hs (sqLoop ≫ sqLoop)
  haveI := hv
  have h2 : 2 ≤ (crossGradingLoc Zbp).codim (sqLoop ≫ sqLoop) := by
    rw [(crossGradingLoc Zbp).codim_comp, crossGradingLoc_sqLoop]
