...
/-!
# Concurrency/Presentation/ChainLocMonoid — a strand component, presented by its own arrows
...
`Machinery/Localization/LocalizationMonoid` applies: the loops there are the monoid presented by
the chain morphisms themselves.
-/
...
`fun a => dimSum a.dims`, and `Machinery/Localization/LocalizationSigma` must see that. -/
