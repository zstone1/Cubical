...
/-!
# Concurrency/Presentation/ArtinPresentation — the transported presentation is the Artin one,
letter for letter

`Machinery/Braid/GermPresentation` presents the base by *all* permutations and the length-additive
products; Matsumoto replaces that by the `n-1` adjacent transpositions and the two Artin families. 
Feeding the second into `chLocPresentation` presents `Ch K[W⁻¹]` with generators `(chain, adjacent
