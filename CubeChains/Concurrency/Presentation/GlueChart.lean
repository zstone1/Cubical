...
/-!
# Concurrency/Presentation/GlueChart — which chart a generator's 1-cell lives over
...
theorem sepCells_glueE (c : ((wedgeHoms K).Elements)ᵒᵖ)
    {a b : (p.fam.obj (eltBase (wedgeHoms K) c)).V}
    (g : (⟨a⟩ : GenObj (p.fam.obj (eltBase (wedgeHoms K) c)).Gen) ⟶ ⟨b⟩) :
    (sepCells K p σ hmix).map (glueE K p.fam c g) = sepVal K p σ c ⟨a⟩ ⟨b⟩ :=
