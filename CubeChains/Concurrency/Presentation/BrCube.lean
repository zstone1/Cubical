...
/-- **A chain of the cube has `n` strands.** -/
...
/-- **The leg of a chain of the cube along a run's arrow** — the run's arrow, read at `□n`.  The
endpoints of `Hom.φ` are given explicitly: they are not read off the expected type. -/
def cubeLeg {n : ℕ} {d : Ch Zbp} (W : ⋁d.dims ⟶ □n) (t : zObj (𝟙^n) ⟶ d) :
    (runCh (Hom.φ t ≫ W) : Ch (□n)) ⟶ (⟨d.dims, W⟩ : Ch (□n)) :=
  ⟨Hom.φ (a := zObj (𝟙^n)) (b := d) t, rfl⟩

@[simp] theorem cubeLeg_φ {n : ℕ} {d : Ch Zbp} (W : ⋁d.dims ⟶ □n) (t : zObj (𝟙^n) ⟶ d) :
    Hom.φ (cubeLeg W t) = Hom.φ t := rfl

/-- **A run's weak-order class over a chain** — the chain's own, crossed by the run's arrow. -/
theorem cross_runCh_comp {n : ℕ} {d : Ch Zbp} (W : ⋁d.dims ⟶ □n) (t : zObj (𝟙^n) ⟶ d) :
    cross (runCh (Hom.φ t ≫ W))
      = cross (⟨d.dims, W⟩ : Ch (□n)) * crossPerm (dimSum_replicate n) t :=
  (cross_eq_mul (cubeLeg W t)).trans
    (congrArg (fun σ => cross (⟨d.dims, W⟩ : Ch (□n)) * σ)
      (crossPerm_eq_of_φ (dimSum_replicate n) (g := cubeLeg W t) (g' := t) (cubeLeg_φ W t)))

/-- …and the crossings add. -/
theorem permLen_cross_runCh_comp {n : ℕ} {d : Ch Zbp} (W : ⋁d.dims ⟶ □n)
    (t : zObj (𝟙^n) ⟶ d) :
  ...
  (congrArg (fun σ => permLen σ + crossLen (⟨d.dims, W⟩ : Ch (□n)))
      (crossPerm_eq_of_φ (dimSum_replicate n) (g := cubeLeg W t) (g' := t)
        (cubeLeg_φ W t))).symm.trans
    (crossLen_eq_add (cubeLeg W t)).symm
...
/-- **The 0-cells over one chain, read in the weak order**: the chain contributes a fixed
permutation `τ`, and each run over it multiplies `τ` on the right by its own crossing,
length-additively. -/
theorem brCube_chain (p : BraidPresentation) {n : ℕ} (c : ((wedgeHoms (□n)).Elements)ᵒᵖ) :
  ...
  refine ⟨?_, ?_⟩
  · rw [← hA, ιV_runPush p (□n) W t, at_ιRun_cube, weakClass, cross_runCh_comp W t, hperm]
    rfl
  · rw [hperm]
    exact (permLen_cross_runCh_comp W t).trans (congrArg permLen (cross_runCh_comp W t))
...
theorem germBrCube_gen (n : ℕ) {A B : GenObj (germBP.Br (□n)).Gen} (e : A ⟶ B) :
  ...
  obtain ⟨τ, hτ⟩ := brCube_chain germBP c
...
theorem artinBrCube_gen (n : ℕ) {A B : GenObj (artinBP.Br (□n)).Gen} (e : A ⟶ B) :
  ...
  obtain rfl : N = n := u.strands.symm.trans (cubeStrands c)
  obtain ⟨τ, hτ⟩ := brCube_chain artinBP c
  obtain ⟨hAeq, hAlen⟩ := hτ v hA
  obtain ⟨hBeq, hBlen⟩ := hτ u hB
