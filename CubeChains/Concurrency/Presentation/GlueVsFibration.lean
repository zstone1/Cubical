...
/-!
# Concurrency/Presentation/GlueVsFibration — what each route's generators perform
...
Both answers are the same atom: `chBraid_runGen` on the glue side, `chBraid_hLocArtinPresentation_arrow`
on the fibration side.  Faithfulness (`eq_of_chBraid_eq`) is what makes that decisive.
-/
...
/-! ## The glue route: the localized slices
...
/-- **The braid a glue-route arrow performs is the one it performs on the elements.** -/
theorem chBraid_locEquivElements_inverse_map {N : ℕ}
  ...
  chBraid_equiv_map (locEquivElements K) (chLocBaseGlueIso K) φ hA hB hA' hB'
