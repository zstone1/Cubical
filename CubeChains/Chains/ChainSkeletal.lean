...
theorem serialWedge_dimSum_eq {ad cd : List ℕ+}
    (φ : BPSet.serialWedge ad ⟶ BPSet.serialWedge cd) :
  ...
  have hmapEq : ∀ (dims : List ℕ+)
      (ψ : (BPSet.serialWedge dims).toPsh ⟶ (BPSet.serialWedge cd).toPsh),
      (wedgeToCubes ⟨dims, ψ⟩).map (fun c => (c.1 : ℕ)) = dims.map (fun d : ℕ+ => (d : ℕ)) := by
    intro dims ψ
    rw [show (fun c : Σ n : ℕ+, (BPSet.serialWedge cd).toPsh.cells (n : ℕ) => (c.1 : ℕ))
  ...
  have hciT : IsCubeChain (BPSet.serialWedge cd).init
    ...
    have h := wedgeToCubes_isCubeChain (K := BPSet.serialWedge cd) cd
      (𝟙 (BPSet.serialWedge cd).toPsh)
    simpa using h
  have hciP : IsCubeChain (BPSet.serialWedge cd).init (wedgeToCubes ⟨ad, φ.hom⟩)
      (BPSet.serialWedge cd).final := by
    have h := wedgeToCubes_isCubeChain (K := BPSet.serialWedge cd) ad φ.hom
    rwa [φ.app_init, φ.app_final] at h
  have hT := isCubeChain_alt_final alt hax
    (wedgeToCubes ⟨cd, 𝟙 (BPSet.serialWedge cd).toPsh⟩) _ _ hciT
...
theorem dimPrefixSum_strictMono {K : BPSet} (cubes : List (Σ n : ℕ+, K.toPsh.cells (n : ℕ)))
...
theorem dimPrefixSum_lt_reflect {K : BPSet} (cubes : List (Σ n : ℕ+, K.toPsh.cells (n : ℕ)))
...
theorem dimPrefixSum_le_reflect {K : BPSet} (cubes : List (Σ n : ℕ+, K.toPsh.cells (n : ℕ)))
...
theorem exists_prefix_bucket {K : BPSet} (L : List (Σ n : ℕ+, K.toPsh.cells (n : ℕ)))
...
theorem blockIdx_surjective {ad cd : List ℕ+}
  ...
  have hlenC : (wedgeToCubes ⟨cd, 𝟙 (BPSet.serialWedge cd).toPsh⟩).length = cd.length :=
  ...
  have htot : dimPrefixSum (wedgeToCubes ⟨ad, φ.hom⟩) (wedgeToCubes ⟨ad, φ.hom⟩).length
      = dimPrefixSum (wedgeToCubes ⟨cd, 𝟙 (BPSet.serialWedge cd).toPsh⟩)
          (wedgeToCubes ⟨cd, 𝟙 (BPSet.serialWedge cd).toPsh⟩).length := by
  ...
  have hm0 : (0 : ℤ) ≤ dimPrefixSum (wedgeToCubes ⟨cd, 𝟙 (BPSet.serialWedge cd).toPsh⟩) j.val := by
    rw [dimPrefixSum]; exact Int.natCast_nonneg _
  have hm : dimPrefixSum (wedgeToCubes ⟨cd, 𝟙 (BPSet.serialWedge cd).toPsh⟩) j.val
  ...
  have hle2 : j.val ≤ (blockIdx φ.hom ⟨i, hiad⟩).val := by
    have hstep : dimPrefixSum (wedgeToCubes ⟨cd, 𝟙 (BPSet.serialWedge cd).toPsh⟩) j.val
        < dimPrefixSum (wedgeToCubes ⟨cd, 𝟙 (BPSet.serialWedge cd).toPsh⟩)
...
/-! ### Bead-count monotonicity of coarsening (for the maximal-coarsening well-foundedness)
...
/-- **A coarsening never increases the bead count.**  `blockIdx f.φ.hom` is a surjection
`Fin a.dims.length ↠ Fin b.dims.length`. -/
theorem ChainCat.dims_length_le_of_hom {K : BPSet} {a b : ChainCat.Obj K} (f : a ⟶ b) :
...
every dimension. -/
theorem ChainCat.dims_eq_of_hom_of_length_eq {K : BPSet} {a b : ChainCat.Obj K} (f : a ⟶ b)
    (hlen : a.dims.length = b.dims.length) : a.dims = b.dims := by
  have hmono := serialWedge_blockIdx_monotone f.φ.hom f.φ.app_init
  have hbij : Function.Bijective (blockIdx f.φ.hom) := by
    rw [Fintype.bijective_iff_surjective_and_card]
    exact ⟨blockIdx_surjective f.φ, by simp [hlen]⟩
  set e := Equiv.ofBijective (blockIdx f.φ.hom) hbij with he
  have h1 : ∀ i, e.symm (blockIdx f.φ.hom i) = i := fun i => e.symm_apply_apply i
  have h2 : ∀ j, blockIdx f.φ.hom (e.symm j) = j := fun j => e.apply_symm_apply j
  have hSmono : Monotone (fun j => e.symm j) := by
    intro x y hxy
    by_contra hc; rw [not_le] at hc
    have hle : blockIdx f.φ.hom (e.symm y) ≤ blockIdx f.φ.hom (e.symm x) := hmono (le_of_lt hc)
    rw [h2, h2] at hle
    have hxy' : x = y := hxy.antisymm hle
    rw [hxy'] at hc; exact absurd hc (lt_irrefl _)
  have hval : ∀ i, (blockIdx f.φ.hom i).val = i.val := fun i =>
    monotone_bij_fin_cast hlen hmono hSmono h1 h2 i
  have hface : ∀ i, (a.dims.get i : ℕ) ≤ (b.dims.get (blockIdx f.φ.hom i) : ℕ) := fun i =>
    StdCube.cells_card_le (StdCube.ev (blockFace f.φ.hom i))
  have hsum_eq : (∑ i : Fin a.dims.length, (a.dims.get i : ℕ))
      = ∑ i : Fin a.dims.length, (b.dims.get (blockIdx f.φ.hom i) : ℕ) := by
    have e1 : (∑ i : Fin a.dims.length, (a.dims.get i : ℕ))
        = (a.dims.map (fun d : ℕ+ => (d : ℕ))).sum := sum_get_eq_sum_map a.dims (fun d => (d : ℕ))
    have e2 : (∑ j : Fin b.dims.length, (b.dims.get j : ℕ))
        = (b.dims.map (fun d : ℕ+ => (d : ℕ))).sum := sum_get_eq_sum_map b.dims (fun d => (d : ℕ))
    have e3 : (∑ i : Fin a.dims.length, (b.dims.get (blockIdx f.φ.hom i) : ℕ))
        = ∑ j : Fin b.dims.length, (b.dims.get j : ℕ) :=
      Fintype.sum_bijective (blockIdx f.φ.hom) hbij _ _ (fun _ => rfl)
    rw [e1, e3, e2, serialWedge_dimSum_eq f.φ]
  have hpt : ∀ i, (a.dims.get i : ℕ) = (b.dims.get (blockIdx f.φ.hom i) : ℕ) := fun i =>
    (Finset.sum_eq_sum_iff_of_le (fun i _ => hface i)).mp hsum_eq i (Finset.mem_univ i)
  apply List.ext_getElem hlen
  intro k h1' h2'
  change a.dims.get ⟨k, h1'⟩ = b.dims.get ⟨k, h2'⟩
  have heq1 : a.dims.get ⟨k, h1'⟩ = b.dims.get (blockIdx f.φ.hom ⟨k, h1'⟩) :=
...
theorem ChainCat.eq_of_hom_of_dims_length_eq {K : BPSet} {a b : ChainCat.Obj K} (f : a ⟶ b)
...
for `HasMaxCoarsening`. -/
theorem ChainCat.lt_dims_length_of_not_isIso {K : BPSet} {a c : ChainCat.Obj K} (g : a ⟶ c)
