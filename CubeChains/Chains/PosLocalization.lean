...
`Chains/GarsideChains` that names the
...
/-- **The Garside presentation of `Ch(Zbp)[W⁻¹]`**: in each degree the interval `1ⁿ ⟶ [n]`,
generators the chain morphisms and relations their factorisations. -/
noncomputable def localizationEquivGarside : (W Zbp).Localization
    ≌ Graded fun n => GarsideMonoid (WStrands Zbp n) (onesObj n) (topObj n) :=
