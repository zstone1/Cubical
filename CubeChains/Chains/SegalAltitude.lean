...
`chSegal (cube n) (serialWedge rest)` invocation gets its `AdmitsAltitude` argument
...
/-- **`trueCount` is additive along the iterated-face map `app`.**  Facing a cell
`x : cells N m` out at the fixed coordinates of `a : cells m k` adds, to the
...
coordinate of `a` (`app_unfold`), exactly mirroring the recursion of `app`. -/
theorem trueCount_app {N m : ℕ} (x : cells N m) :
    ∀ {k : ℕ} (a : cells m k),
      trueCount (app (K := stdPre N) x a) = trueCount x + trueCount a := by
  intro k a
  ...
  | _ d ih =>
    ...
    · -- peel the smallest fixed coordinate of `a`
      ...
      change trueCount (face (minFixedVal a h) (minFixedIdx a h)
          (app (K := stdPre N) x (freeMin a h))) = trueCount x + trueCount a
...
`PrecubicalConstructions` map `stdPre m ⟶ stdPre N`; `StdCube.ev` reads off its
top-cell value in `cells N m`, and `trueCount` counts that cell's `1`-coordinates. -/
noncomputable def cubeAlt (N : ℕ) : ∀ m, (cube N).toPsh.cells m → ℤ :=
  fun _ x => (StdCube.trueCount (StdCube.ev x) : ℤ)

/-- The face map of `cube N` is precomposition by the coface (Yoneda). -/
theorem cube_faceMap (N : ℕ) {m : ℕ} (ε : Bool) (i : Fin (m + 1))
    (x : (cube N).toPsh.cells (m + 1)) :
    (cube N).toPsh.faceMap ε i x = PrecubicalSet.coface ε i ≫ x := rfl
...
theorem cube_alt_axiom (N : ℕ) {m : ℕ} (ε : Bool) (i : Fin (m + 1))
    (x : (cube N).toPsh.cells (m + 1)) :
    cubeAlt N m ((cube N).toPsh.faceMap ε i x)
      = cubeAlt N (m + 1) x + (if ε then 1 else 0) := by
  change (trueCount (StdCube.ev ((cube N).toPsh.faceMap ε i x)) : ℤ)
    = (trueCount (StdCube.ev x) : ℤ) + (if ε then 1 else 0)
  -- `ev (coface ε i ≫ x) = app x (ev (coface ε i)) = app x (face ε i (topCell (m+1)))`
  have hev : StdCube.ev ((cube N).toPsh.faceMap ε i x)
      = StdCube.app (StdCube.ev x) (StdCube.face ε i (StdCube.topCell (m + 1))) := by
    -- `ev (coface ≫ x) = Hom.app x (ev coface) = Hom.app x (face ε i ⊤)`;
    -- and `Hom.app x = Hom.app (canonicalMap (ev x)) = app (ev x)` (cube Yoneda).
    have h1 : StdCube.ev ((cube N).toPsh.faceMap ε i x)
        = PrecubicalConstructions.Hom.app x m (StdCube.ev (PrecubicalSet.coface ε i)) :=
      ev_comp (PrecubicalSet.coface ε i) x
    rw [h1, ev_coface]
    exact app_unique (c := StdCube.ev x) x rfl (StdCube.face ε i (StdCube.topCell (m + 1)))
...
theorem cube_admitsAltitude (N : ℕ) : (cube N).AdmitsAltitude := by
  refine ⟨cubeAlt N, fun ε i x => cube_alt_axiom N ε i x, ?_⟩
  -- `(cube N).init = canonicalMap (constVertex N false)`, `ev` of which is that vertex.
  change (StdCube.trueCount (StdCube.ev ((cube N).init)) : ℤ) = 0
  rw [show (cube N).init = StdCube.canonicalMap (StdCube.constVertex N false) from rfl,
...
theorem wedge2Alt_cocone
    (altX : ∀ n, X.toPsh.cells n → ℤ) (altY : ∀ n, Y.toPsh.cells n → ℤ)
    (hY0 : altY 0 Y.init = 0) (m : ℕ) :
    X.finalVertex.app (op (Box.ob m)) ≫ TypeCat.ofHom (altX m)
      = Y.initVertex.app (op (Box.ob m))
  ...
  · subst hm
    have hv : v = 𝟙 (Box.ob 0) :=
      Subsingleton.elim (α := (BPSet.cube 0).toPsh.cells 0) _ _
    have hxf : ConcreteCategory.hom (X.finalVertex.app (op (Box.ob 0))) v = X.final := by
      change (yonedaEquiv.symm X.final).app (op (Box.ob 0)) v = X.final
      rw [yonedaEquiv_symm_app_apply, hv, op_id, X.toPsh.map_id]
      rfl
    have hyi : ConcreteCategory.hom (Y.initVertex.app (op (Box.ob 0))) v = Y.init := by
      change (yonedaEquiv.symm Y.init).app (op (Box.ob 0)) v = Y.init
...
theorem wedge2Alt_inl (altX : ∀ n, X.toPsh.cells n → ℤ) (altY : ∀ n, Y.toPsh.cells n → ℤ)
    (hY0 : altY 0 Y.init = 0) {m : ℕ} (x : X.toPsh.cells m) :
...
theorem wedge2Alt_inr (altX : ∀ n, X.toPsh.cells n → ℤ) (altY : ∀ n, Y.toPsh.cells n → ℤ)
    (hY0 : altY 0 Y.init = 0) {m : ℕ} (y : Y.toPsh.cells m) :
    wedge2Alt altX altY hY0 m
...
theorem wedge2_inl_faceMap {m : ℕ} (ε : Bool) (i : Fin (m + 1)) (x : X.toPsh.cells (m + 1)) :
    (wedge2 X Y).toPsh.faceMap ε i
        ((pushout.inl X.finalVertex Y.initVertex).app (op (Box.ob (m + 1))) x)
...
theorem wedge2_inr_faceMap {m : ℕ} (ε : Bool) (i : Fin (m + 1)) (y : Y.toPsh.cells (m + 1)) :
    (wedge2 X Y).toPsh.faceMap ε i
        ((pushout.inr X.finalVertex Y.initVertex).app (op (Box.ob (m + 1))) y)
...
theorem wedge2_admitsAltitude (hX : X.AdmitsAltitude) (hY : Y.AdmitsAltitude) :
  ...
  · -- basepoint: `(wedge2 X Y).init = inl X.init`, altitude `altX X.init = 0`.
    rw [show (wedge2 X Y).init
        = (pushout.inl X.finalVertex Y.initVertex).app (op (Box.ob 0)) X.init from rfl,
      wedge2Alt_inl]
    exact hX0
...
sequence: `□⁰` is `cube 0`, and `□^∨(n :: rest) = (cube n) ∨ □^∨(rest)` admits an
altitude by `wedge2_admitsAltitude` from the head cube (`cube_admitsAltitude`) and the
tail (the recursive call). -/
theorem serialWedge_admitsAltitude : ∀ dims : List ℕ+, (serialWedge dims).AdmitsAltitude
