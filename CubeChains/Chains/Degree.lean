...
/-!
# Chains/Degree — the grading on `Ch K`, and the codimension of a refinement
...
`chFunctor` is lax monoidal (`Chains/WedgeLaxMonoidal`) and `gradeFunctor`, the constant functor at
the delooping of `(ℕ, +)`, is lax monoidal by addition.  `codimNat` is `codimFunctor` read as a
