...
import CubeChains.Braid.Sum

/-!
# Chains/WedgeBraid — the braid grading of `Ch K`, from the coordinate map alone
...
`ConcPos K` (`Salvetti/EventBraid`) orders events by the run, so it agrees with `chBraid` exactly
where the run is recoverable from the wedge map — the `H`-model question of whether the grading of
`Ch⋆ K` factors along `Ch (Hbp K) ⥤ Ch Zbp`. -/
