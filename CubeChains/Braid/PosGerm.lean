...
`PosBraid n` is the germ presentation of `Braid/Germ` read as a **monoid**: the same generators and
the same length-additive relations, with no inverses.
...
/-- The underlying permutation of a positive braid — `Braid/Germ`'s `permHom`, on the monoid. -/
def posPermHom (n : ℕ) : PosBraid n →* Perm (Fin n) :=
